...
class HestonPathsGenerator(BasePrimary):
    ...
    def step(self) -> tuple[Tensor, Tensor]:
        ...
        self.S *= torch.exp(
            (self.mu - 0.5 * self.v) * self.dt +
            torch.sqrt(self.v) * dW1
        )
        self.current_step += 1
        return self.S.clone(), self.v.clone()
    ...
    def simulate(self, n_paths: int, duration: float) -> None:
        ...
        S_paths = torch.empty((self.n_steps + 1, n_paths),
                              dtype=self.dtype, device=self.device)
        v_paths = torch.empty((self.n_steps + 1, n_paths),
                              dtype=self.dtype, device=self.device)
